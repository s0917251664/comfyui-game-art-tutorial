...
- `tools_src/comfyui_pipeline/profiles/*.json` —— **圖片模型設定檔**(模型檔名、取樣參數、依記憶體的預設解析度、可用 task、各平台驗證狀態),全平台共用、進版控,隨 `comfyui_pipeline/` 一起部署;每台機器只是依 `device_config.json` 的 tier 選用其中一份。**要換圖片模型檔名或取樣參數,改設定檔,不要改 `image_graphs.py`**。使用者明確要在大機器用小管線時,用 `generate.py --profile <id>` 或 `detect_image_capabilities.py --default-profile <id>`,不要手改 `device_config.json`。設計見 `docs/model-profiles-design.md`

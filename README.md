...
同一角色要製作 Idle、Win、Fail 等一整組遊戲動作時，不要把每支影片當成互不相關的臨時呼叫。先依 [`skills/comfyui-character-animation-workflow/SKILL.md`](skills/comfyui-character-animation-workflow/SKILL.md) 建立動作表，鎖定代表動作，再逐支走既有 task 的技術契約與人工驗收；生成時可先準備 frames，但動作接受後才把 frames 或合成檔列為正式交付。若尚未準備 frames，接受後直接從既有 MP4 抽幀，不為抽幀重新生成。這個流程不新增生成 backend，也不代表目前已支援 APNG、透明影片或第三方付費 provider。
...
- [單角色動畫工作流程](skills/comfyui-character-animation-workflow/SKILL.md)：動作表、代表動作、逐支技術／人工驗收與接受後交付抽幀

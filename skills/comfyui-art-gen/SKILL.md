...
每次圖片 task 完成後，依這個順序驗收；這是把上面的既有原則收成固定檢查，不新增模型分數或通用門檻：

1. **輸出契約**：確認實際檔案存在、尺寸符合本次要求；要求去背或 `icon_asset` 時，確認是 RGBA 且 alpha 真的有透明區域。離線測試不能取代這一步。
2. **需求特徵**：實際打開圖片，逐項比對使用者明確指定的角色、顏色、姿勢、構圖、物件數量與不想出現的內容。未指定的美術偏好不要自行補成驗收標準。
3. **task 控制目的**：`pose_only`／`character_action` 看姿勢與文字是否一致；`style_lock` 看角色／風格參考是否仍可辨識；`inpaint`／`guided_inpaint` 看修改是否集中在遮罩區，並留意遮罩外被牽動；`upscale` 看構圖是否仍是同一張圖。
4. **可追溯限制**：遇到已知限制時依 `reference/known-limitations.md` 回報，不靠換 seed 或重複生成假裝問題已解決。

如果內容不符合，先依本技能既有參數語意判斷是否能用一次有理由的調整重跑；無法對應到現有 task／參數時停止，不臨場新增 graph、fallback 或隱性預設。

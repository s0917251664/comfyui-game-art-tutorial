...
### 5. workflows/ 同步(這個新能力有對應的手動節點操作方式時)

- [ ] 如果 `generate.py` 的新邏輯有辦法讓人在 ComfyUI 網頁介面手動組出對應節點圖,補一份 `workflows/*.json`,命名跟現有檔案的慣例(`ChN[字母]_描述.json`)對齊
- [ ] 這份 workflow JSON 手動寫或修改後,**一定要寫程式驗證過**再交付:每個節點的 `pos` 是不是恰好 2 個數字、`last_link_id` 是否等於實際最大 link id、每條 link 兩端指到的節點 id 是否存在——這些是這個專案實際踩過的低級錯誤(手滑寫壞格式),不要重蹈覆轍
- [ ] 暫時沒空補這份 workflow 的話,不要悶著不講,在對應文件明確標註「這個缺口還沒補」,讓使用者知道現況,而不是留一個沒人知道的斷點
